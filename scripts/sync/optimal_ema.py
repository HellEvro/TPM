...
2. ✅ БОЛЬШЕ ДАННЫХ: Анализирует до 10000 свечей (увеличено с 5000) для большего 
   охвата истории. Свечи загружаются через API, но сначала проверяется кэш в памяти 
   (coins_rsi_data['candles_cache']) и файле (data/candles_cache.json) для ускорения.
...
3. Кэш свечей в файле: data/candles_cache.json
   - Создается автоматически при загрузке свечей через API
   - Используется для ускорения последующих запусков скрипта
   - Свечи считаются актуальными до 24 часов
...
ПРИМЕЧАНИЕ: Скрипт сначала проверяет кэш свечей (память → файл), и только 
если кэш недоступен или устарел, загружает свечи через API. Это значительно 
ускоряет работу при повторных запусках.
"""
                    ...
                    if symbol_key in file_cache:
                        ...
                        if 'candles' in cached_data and cached_data.get('timeframe') == self.timeframe:
                            ...
                            if age_hours < 24 and candles and len(candles) >= MIN_CANDLES_FOR_ANALYSIS:
                                logger.info(f"✅ Использованы свечи из файла кэша: {len(candles)} свечей для {symbol} (возраст: {age_hours:.1f}ч)")
                                return candles
