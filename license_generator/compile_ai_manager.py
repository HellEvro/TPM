"""
Скрипт для компиляции ai_manager_source.py в .pyc
"""
...
def compile_ai_manager():
    """Компилирует ai_manager_source.py в .pyc"""
    
    print("=" * 60)
    print("COMPILING AI MANAGER")
    print("=" * 60)
    print()
    
    # Исходный файл
    source_file = Path('license_generator/source/ai_manager_source.py')
    ...
    try:
        ...
        compiled_file = target_dir / '__pycache__' / 'ai_manager.cpython-{}{}.opt-2.pyc'.format(
            sys.version_info.major, sys.version_info.minor
        )
        
        if not compiled_file.exists():
            # Пробуем без opt-2
            compiled_file = target_dir / '__pycache__' / 'ai_manager.cpython-{}{}.pyc'.format(
                sys.version_info.major, sys.version_info.minor
            )
        
        if compiled_file.exists():
            # Копируем как ai_manager.pyc в целевую директорию
            ...
            print()
            print("=" * 60)
            print("[OK] AI MANAGER COMPILED SUCCESSFULLY")
            print("=" * 60)
            print()
