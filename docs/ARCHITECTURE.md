...
3. Для каждой монеты:
   ...
   ├─ check_coin_maturity()           ← Зрелая/Незрелая (для UI, НЕ блокирует)
...
5. Если Auto Bot enabled:
   process_auto_bot_signals()
   └─ Если сигнал есть → create_bot(symbol)
   ↓
6. NewTradingBot управляет позицией
   ├─ _enter_position(side) вызывается
   │  ├─ ✅ ВСЕ ФИЛЬТРЫ проверяются здесь:
   ...
   │  │  ├─ 🤖 AI Optimal Entry Detection (если включен)
   │  │  ├─ 🤖 AI Risk Management - адаптация размера
   │  │  └─ Открытие позиции на бирже
   │  ├─ 🤖 AI Risk Management - адаптация стоп-лосса
   │  ├─ Установка stop-loss
   │  ├─ Trailing stop при прибыли
   │  └─ Закрытие по RSI выхода
```

**Ключевой принцип:** Все блокирующие фильтры (ExitScam, RSI Time Filter, Maturity) проверяются ТОЛЬКО при входе в позицию в `_enter_position()`, а не для всех монет постоянно. Это оптимизация производительности и правильная логика работы фильтров как "отсекателя".
