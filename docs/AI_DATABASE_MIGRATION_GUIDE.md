...
**Важно:** Это руководство описывает миграцию AI модуля (файлы в `data/ai/`). Bots модуль (`bots.py`) также полностью мигрирован на БД по аналогии с AI модулем.

### ✅ Статус миграции

- **AI модуль:** Файлы в `data/ai/` → БД `data/ai/ai_data.db` ✅ **Полностью мигрировано**
- **Bots модуль:** Файлы в `data/` (не в `data/ai/`) → БД `data/bots_data.db` ✅ **Полностью мигрировано**
  - `data/bots_state.json` → `bots_state` таблица
  - `data/bot_positions_registry.json` → `bot_positions_registry` таблица
  - `data/rsi_cache.json` → `rsi_cache` таблица
  - `data/mature_coins.json` → `mature_coins` таблица
  - И другие файлы в `data/` → соответствующие таблицы

**Bots.py НЕ читает файлы из `data/ai/`** - это отдельная папка только для AI модуля!
Оба модуля используют одинаковую архитектуру БД с одинаковыми улучшениями (проверка целостности, резервное копирование, retry логика).
...
### Файлы bots.py (уже мигрированы ✅)

Bots модуль полностью мигрирован на БД `data/bots_data.db`:

| Файл (data/) | Таблица БД | Статус |
|-------------|-----------|--------|
| `bots_state.json` | `bots_state` | ✅ Полностью мигрировано |
| `bot_positions_registry.json` | `bot_positions_registry` | ✅ Полностью мигрировано |
| `rsi_cache.json` | `rsi_cache` | ✅ Полностью мигрировано |
| `mature_coins.json` | `mature_coins` | ✅ Полностью мигрировано |
| `process_state.json` | `process_state` | ✅ Полностью мигрировано |
| `individual_coin_settings.json` | `individual_coin_settings` | ✅ Полностью мигрировано |
| `maturity_check_cache.json` | `maturity_check_cache` | ✅ Полностью мигрировано |
| `delisted.json` | `delisted` | ✅ Полностью мигрировано |

**Примечание:** `bots_data.json` в `data/ai/` - это снимки данных ботов, собираемые AI модулем для обучения. Это НЕ то же самое, что `bots_state.json` в `data/`, который использовался самим `bots.py` (теперь заменен на БД).
...
**Проблема:** При обнаружении повреждений БД требовалось ручное вмешательство. Также возникали проблемы при блокировке файла другим процессом при создании резервной копии.

**Решение:** Автоматическое исправление через VACUUM или восстановление из резервной копии. Улучшенная логика с retry для резервных копий и автоматическим использованием существующих копий.

#### Улучшенное резервное копирование с retry логикой
...
1. **Retry для резервных копий:** До 3 попыток с экспоненциальной задержкой (1s, 2s, 3s)
2. **Использование существующих копий:** Если не удалось создать новую копию, использует существующие
3. **Пропуск VACUUM:** При критических повреждениях (`malformed`) сразу переходит к восстановлению
4. **Умное восстановление:** Выбирает более старую копию, если была создана новая поврежденная
5. **Подробные инструкции:** При невозможности восстановления дает четкие рекомендации

### 🚨 Автоматическая обработка критических ошибок БД

**Проблема:** При работе с БД (особенно на сетевых дисках через UNC пути) могут возникать критические ошибки:
- `database disk image is malformed` - БД повреждена
- `disk I/O error` - ошибка ввода-вывода (сеть, диск)

**Решение:** Автоматическое обнаружение и исправление этих ошибок прямо в `_get_connection` с автоматическим повтором операции после восстановления.

#### Реализация в _get_connection
...
def _get_connection(self, retry_on_locked: bool = True, max_retries: int = 5):
    ...
    for attempt in range(max_retries if retry_on_locked else 1):
        try:
            ...
            elif "file is not a database" in error_str:
...
#### Преимущества автоматической обработки ошибок

1. **Автоматическое обнаружение:** Система автоматически обнаруживает критические ошибки (`malformed`, `I/O error`)
2. **Автоматическое исправление:** При обнаружении ошибки автоматически запускается `_repair_database()`
3. **Автоматический повтор:** После исправления операция автоматически повторяется один раз
4. **Защита данных:** Перед исправлением создается резервная копия
5. **Подробное логирование:** Все действия логируются для диагностики

#### Когда это особенно важно

- **UNC пути (сетевые диски):** При работе с БД на сетевых дисках возможны временные проблемы с I/O из-за сетевых задержек
- **Параллельный доступ:** При доступе к одной БД с нескольких серверов могут возникать конфликты
- **Сбои питания:** При некорректном завершении работы БД может быть повреждена
- **Проблемы с диском:** При проблемах с физическим диском или файловой системой

#### Примеры логов

```
[AI] 04:57:30 - ERROR - ❌ КРИТИЧНО: Ошибка I/O при работе с БД: disk I/O error
[AI] 04:57:30 - WARNING - 🔧 Попытка автоматического исправления...
[AI] 04:57:30 - WARNING - 🔧 Попытка исправления БД...
[AI] 04:57:30 - INFO - 💾 Создана резервная копия БД: data/ai/ai_data.db.backup_20250101_045730
[AI] 04:57:31 - INFO - 🔧 Выполняю VACUUM для исправления БД (это может занять время)...
[AI] 04:57:35 - INFO - ✅ VACUUM выполнен
[AI] 04:57:35 - INFO - ✅ БД успешно исправлена с помощью VACUUM
[AI] 04:57:35 - INFO - ✅ БД исправлена, повторяем операцию...
```

#### Важные замечания

1. **Один раз:** Исправление выполняется только один раз за операцию (при `attempt == 0`)
2. **Резервная копия:** Перед исправлением всегда создается резервная копия
3. **Задержка:** После исправления делается задержка 1 секунда перед повтором операции
4. **Логирование:** Все критические ошибки и действия логируются
...
2. ✅ **Резервное копирование** перед удалением поврежденной БД с retry логикой (до 3 попыток)
...
9. ✅ **Автоматическая обработка критических ошибок** (`database disk image is malformed`, `disk I/O error`) в `_get_connection` с автоматическим исправлением и повтором операции
10. ✅ **Умное восстановление:** Автоматическое использование существующих резервных копий, если не удалось создать новую
11. ✅ **Пропуск VACUUM:** При критических повреждениях сразу переходит к восстановлению из резервной копии
...
6. **Автоматическую обработку критических ошибок** в `_get_connection`:
   - `database disk image is malformed`
   - `disk I/O error`
   - Автоматическое исправление через `_repair_database()`
   - Автоматический повтор операции после исправления
7. **Retry логику для резервных копий** при блокировке файла (до 3 попыток)
8. **Умное восстановление** из существующих резервных копий, если не удалось создать новую
...
class BotsDatabase:
    ...
    def _get_connection(self, retry_on_locked: bool = True, max_retries: int = 5):
        """
        ВАЖНО: Обязательно реализуйте обработку критических ошибок!
        
        При ошибках 'database disk image is malformed' или 'disk I/O error':
        1. Автоматически вызывайте _repair_database()
        2. Повторяйте операцию один раз после исправления
        3. Логируйте все действия для диагностики
        """
        # Реализация с обработкой критических ошибок (см. пример выше)
        # ...
```
