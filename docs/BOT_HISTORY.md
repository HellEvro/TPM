...
{
  "history": [
    // Массив всех действий (без ограничений - для обучения ботов)
  ],
  "trades": [
    // Массив всех сделок (без ограничений - для обучения ботов)
  ],
...
- ⚠️ **Ограничения отключены** - все записи сохраняются для обучения ботов
- История: без ограничений (ранее было 10000 записей)
- Сделки: без ограничений (ранее было 5000 записей)
- Для изменения лимитов: установите `MAX_HISTORY_ENTRIES` и `MAX_TRADE_ENTRIES` в `bot_engine/bot_history.py`
