...
Менеджер позволяет:
- устанавливать или обновлять зависимости (`pip install -r requirements.txt`) без виртуального окружения — основной сценарий
- запускать/останавливать `app.py`, `bots.py`, `ai.py`
- работать с виртуальным окружением `.venv` (создание/обновление, удаление) при необходимости
- проверять обновления из Git
- получать Hardware ID, импортировать `.lic` файл
- открывать конфигурации и журналы
- автоматически выполнить `git init` и добавление `origin` (`git@github.com:HellEvro/TPM_Public.git`), если `.git` отсутствует
