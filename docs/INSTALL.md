...
2. **Установка зависимостей напрямую** — необязательно (`pip install -r requirements.txt` в системный Python; в этом режиме на Windows/Python 3.13 менеджер тоже обходит `coincurve`)
3. **Обновления из Git** — проверка/обновление
4. **Лицензия и ключи (опционально)** — Hardware ID, импорт `.lic`
5. **Запуск сервисов** — `app.py`, `bots.py`, `ai.py`
...
9. На Windows с Python 3.13 менеджер автоматически избегает установки `coincurve` (опциональной зависимости `ccxt`) и при работе через `.venv`, и при прямой установке.
