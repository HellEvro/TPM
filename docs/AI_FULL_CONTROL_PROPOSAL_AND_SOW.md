...
**Версия:** 0.3 (вариант B; отдельный конфиг и таблица FullAI; при выключении FullAI — возврат к обычному режиму без изменений пользовательского конфига)  
**Дата:** 17.02.2026
...
| **Отдельный конфиг и таблица FullAI** | Для режима FullAI используются **отдельный конфиг** (который ИИ может менять как хочет в рамках пространства, заданного пользователем) и **отдельная таблица в БД** для параметров по монетам в FullAI. Обычный конфиг пользователя и `individual_coin_settings` **не затрагиваются**. При выключении FullAI бот сразу возвращается к обычному режиму: используются только пользовательский конфиг и обычные настройки монет — ничего возвращать не нужно, т.к. они не перезаписывались. |
| Конфиг — пространство возможных параметров | В **пользовательском** конфиге задаются возможные параметры (диапазоны, опции). ИИ в FullAI меняет только **копию/отдельный конфиг FullAI** и данные в **таблице FullAI** по монетам. Единственное жёсткое исключение — таймфрейм (всегда из пользовательского конфига). |
| Параметры на монету в БД (FullAI) | ИИ пишет и обновляет настройки по каждой монете в **отдельной таблице FullAI** (например `full_ai_coin_params` или `prii_coin_params`); при включённом FullAI используются только она и конфиг FullAI. Таблица `individual_coin_settings` для обычного режима не используется и не изменяется в FullAI. |
...
- **Параметры:** ИИ меняет только **отдельный конфиг FullAI** и данные в **отдельной таблице FullAI** по монетам. Пользовательский конфиг и `individual_coin_settings` не трогаются. Таймфрейм всегда берётся только из пользовательского конфига.
- Скрипт исполняет ордера; при FullAI читает/пишет только конфиг FullAI и таблицу FullAI.
...
- **Консистентность:** переключение режима (FullAI ↔ стандарт) не должно ломать уже открытые позиции; выход по уже открытым может идти по старым правилам до закрытия.
- **При выключении FullAI:** все настройки, которые были до включения FullAI, остаются в силе — пользовательский конфиг и `individual_coin_settings` не менялись. Используются снова только они; конфиг FullAI и таблица FullAI просто перестают использоваться. Возврат к обычному режиму автоматический, без шага «восстановить конфиг».
...
### 2.2 Отдельный конфиг FullAI и отдельная таблица в БД (обычный режим не затрагивается)

| № | Задача | Детали |
|---|--------|--------|
| 2.2.1 | Отдельный конфиг FullAI | Хранить **отдельный конфиг для FullAI** (например в БД или в отдельном файле `configs/full_ai_config.json` / ключ в bots_data). При включении FullAI он инициализируется копией пользовательского конфига (или пространства параметров из него); далее ИИ меняет только этот конфиг. Пользовательский `auto_bot_config` / `bot_config.py` **не перезаписываются**. Таймфрейм в конфиг FullAI не копируется как изменяемый — всегда читается из пользовательского конфига. |
| 2.2.2 | Отдельная таблица FullAI по монетам | **Отдельная таблица в БД** только для FullAI, например `full_ai_coin_params` или `prii_coin_params` (symbol, params_json или нормализованные поля, updated_at, created_at). Используется **только** при `full_ai_control === true`. Таблица `individual_coin_settings` для обычного режима не читается и не записывается в FullAI. Так обычный режим не затрагивается. |
| 2.2.3 | Миграция и API | Миграция в `bots_database.py` на создание таблицы FullAI при отсутствии. Методы: `save_full_ai_coin_params(symbol, params)`, `load_full_ai_coin_params(symbol)`, `load_all_full_ai_coin_params()`. При необходимости — эндпоинты GET/POST для одной монеты и списка (по аналогии с individual-settings), с проверкой что данные относятся к FullAI. |
| 2.2.4 | При выключении FullAI | При переключении `full_ai_control` в `false`: в рантайме перестать использовать конфиг FullAI и таблицу FullAI; использовать только пользовательский конфиг и `individual_coin_settings`. Никакого шага «вернуть конфиг» не требуется — пользовательский конфиг не менялся. Конфиг FullAI и таблица FullAI остаются в БД/файле для следующего включения FullAI. |
...
| 2.3.2 | Источник параметров при FullAI | Для полного ИИ использовать только **конфиг FullAI** и **таблицу FullAI** (full_ai_coin_params) для данной монеты; если записи по монете нет — fallback на конфиг FullAI или дефолты из пространства параметров пользователя. Пользовательский конфиг и individual_coin_settings не читать в ветке FullAI. |
| 2.3.3 | Фильтры и лимиты | Делистинг и прочие нефункциональные проверки сохраняются. Лимиты задаются пользовательским конфигом как пространство; ИИ выбирает значения в рамках конфига FullAI и таблицы FullAI. Таймфрейм **всегда** только из пользовательского конфига. |

### 2.4 Логика выхода (вариант B: ИИ решает «закрыть сейчас»)

| № | Задача | Детали |
|---|--------|--------|
| 2.4.1 | ИИ решает о закрытии позиции | В режиме FullAI при каждой проверке выхода вызывать ИИ: `get_ai_exit_decision(symbol, position, candles, pnl_percent, prii_config, full_ai_coin_params)` → `{ close_now: bool, reason?, confidence? }`. Если `close_now === true` — закрывать позицию. Классическая логика выхода по RSI/trailing/break_even не используется; решение принимает только ИИ. Параметры по монете — из таблицы FullAI. |
| 2.4.2 | Параметры выхода от ИИ | ИИ хранит в **таблице FullAI** (full_ai_coin_params) любые нужные ему параметры выхода в пределах пространства, заданного пользовательским конфигом. Таймфрейм в таблице FullAI не хранится — используется только таймфрейм из пользовательского конфига. |
...
| 2.5.3 | Выводы и доработка стратегии | На основе оценок по сделкам: (1) **по символу** — корректировать параметры входа/выхода и сохранять **только в таблице FullAI** (full_ai_coin_params) и/или в конфиге FullAI; (2) при необходимости — формировать выборки для переобучения моделей в ai.py. Логировать выводы. Пользовательский конфиг и individual_coin_settings не изменять. |
...
| № | Задача | Детали |
|---|--------|--------|
| 2.6.1 | Кто пишет в конфиг FullAI и таблицу FullAI | Оптимизатор и/или модуль изучения сделок записывают параметры **только в конфиг FullAI** и в **таблицу FullAI** (full_ai_coin_params). Пользовательский конфиг и `individual_coin_settings` не трогаются. |
| 2.6.2 | Использование истории сделок | Для обучения/оптимизации и для модуля изучения сделок брать данные из `bot_trades_history` (bots_data.db); при необходимости — из ai_data.db и с биржи. Результаты обучения писать только в конфиг FullAI и таблицу FullAI. |
| 2.6.3 | Пространство параметров и таймфрейм | Диапазоны и опции задаются в **пользовательском** конфиге. ИИ меняет только конфиг FullAI и таблицу FullAI в рамках этого пространства. **Таймфрейм не входит в пространство изменяемых ИИ параметров:** задаётся только пользователем в конфиге и при записи в конфиг FullAI / таблицу FullAI не переопределяется. |

### 2.7 Контракты ИИ (вход и выход)

**Вход (псевдокод):**

- **Вход:** `symbol`, `direction` ('LONG'|'SHORT'), `candles`, `current_price`, `prii_config` (конфиг FullAI), per-coin из таблицы FullAI (full_ai_coin_params), опционально `coin_data`. **Таймфрейм** всегда из пользовательского конфига.
...
- **Вход:** `symbol`, `position`, `candles`, `pnl_percent`, `prii_config`, таблица FullAI для символа. Свечи и индикаторы — на **таймфрейме из пользовательского конфига**.
...
| Отдельный конфиг FullAI + таблица FullAI в БД, миграция, API | 6–10 | Конфиг FullAI (файл/БД); таблица full_ai_coin_params; при выключении FullAI — только переключение источника, без восстановления |
...
| ИИ меняет только конфиг FullAI и таблицу FullAI (кроме таймфрейма) | 4–8 | Чтение/запись конфига FullAI и full_ai_coin_params; таймфрейм только из пользовательского конфига |
| Изучение сделок + доработка стратегии (только FullAI-хранилище) | 12–20 | Обновление конфига FullAI и таблицы FullAI; пользовательский конфиг и individual_coin_settings не трогать |
| Обучение/запись в конфиг FullAI и таблицу FullAI | 8–16 | Оптимизатор и модуль изучения сделок пишут только в FullAI-хранилище |
...
1. Реализовать **отдельный конфиг FullAI** и **отдельную таблицу в БД** для FullAI (например `full_ai_coin_params`). Переключатель «Полный Режим ИИ» на вкладке Управление уже добавлен.  
2. При включении FullAI: использовать только конфиг FullAI и таблицу FullAI; при выключении — сразу переходить на пользовательский конфиг и `individual_coin_settings` (возврат без шага «восстановить»).  
3. Реализовать `get_ai_entry_decision` и ветку в `should_open_long`/`should_open_short` (источник параметров — конфиг FullAI и таблица FullAI).  
...
5. Таймфрейм всегда только из пользовательского конфига; ИИ меняет только конфиг FullAI и таблицу FullAI.  
6. Модуль изучения сделок и оптимизатор пишут только в конфиг FullAI и таблицу FullAI.  
7. Обработка невалидной лицензии и индикация в UI.

**Зафиксировано:** вариант B; таймфрейм только пользователь; **отдельный конфиг FullAI + отдельная таблица FullAI** — обычный режим не затрагивается, при выключении FullAI все настройки «как до включения» (пользовательский конфиг не менялся).
