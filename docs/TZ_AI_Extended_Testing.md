...
- `ai_trainer` теперь использует `ProtectionState` + `evaluate_protections()` для закрытий → симуляции повторяют поведение `NewTradingBot`.
- `NewTradingBot.check_protection_mechanisms()` подключён к ядру, идёт адаптация обновления биржевых стопов.
- TODO: вынести exit-scam/time filters и применить в `ai_backtester_new`.
