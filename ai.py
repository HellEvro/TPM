...
class AISystem:
    ...
    def start(self):
        ...
        # ВАЖНО: Загружаем полную историю свечей в фоне (не блокируем запуск)
        # Обучение продолжается с доступными данными (candles_cache.json)
        if self.data_collector:
            full_history_file = os.path.join('data', 'ai', 'candles_full_history.json')
            if not os.path.exists(full_history_file):
                logger.info("📊 Запускаем загрузку полной истории свечей в фоне...")
                logger.info("   💡 Это может занять несколько минут (загружаем до 2000 свечей для каждой монеты с пагинацией)")
                logger.info("   💡 Обучение продолжается с доступными данными (candles_cache.json)")
                logger.info("   💡 Полная история будет использована при следующем обучении")
                
                # Загружаем в фоне - НЕ блокируем запуск
                    ...
                    try:
                        ...
                        for attempt in range(max_retries):
                            try:
                                ...
                                if success:
                                    logger.info("✅ Полная история свечей загружена в фоне")
                                    return
                                else:
                                ...
                                if attempt < max_retries - 1:
                                    ...
                                    logger.debug(f"   ⚠️ Не удалось загрузить полную историю после {max_retries} попыток")
                ...
                logger.info("   ✅ Фоновая загрузка свечей запущена")
            else:
        ...
        # ВАЖНО: Сначала собираем начальные данные перед обучением и бэктестом
        if self.data_collector:
