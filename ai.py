...
class AISystem:
    ...
    def start(self):
        ...
        # ВАЖНО: Сначала загружаем полную историю свечей (если еще не загружена)
        # Только после этого запускаем обучение и бэктест
        ...
        if self.data_collector:
            ...
            if not os.path.exists(full_history_file):
                ...
                logger.info("   ⏳ Ожидаем завершения загрузки перед запуском обучения и бэктеста...")
                
                # Загружаем синхронно - ждем завершения
                success = self.data_collector.load_full_candles_history()
                if success:
                    logger.info("✅ Полная история свечей загружена")
                    candles_loaded = True
                else:
                    logger.warning("⚠️ Не удалось загрузить полную историю (возможно bots.py не запущен)")
                    logger.info("💡 Продолжаем работу с доступными данными")
            else:
