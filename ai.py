            ...
            try:
                ...
                if current_time - last_training_time >= self.config['training_interval']:
                    if self.trainer:
                        ...
                        # ВАЖНО: Перед каждым обучением дозагружаем новые свечи
                        logger.info("=" * 80)
                        logger.info("📊 ОБНОВЛЕНИЕ СВЕЧЕЙ ПЕРЕД ОБУЧЕНИЕМ")
                        logger.info("=" * 80)
                        logger.info("   💡 Дозагружаем только новые свечи (инкрементальное обновление)")
                        logger.info("   💡 Это обеспечит актуальные данные для обучения")
                        try:
                            if self.data_collector:
                                update_success = self.data_collector.load_full_candles_history()
                                if update_success:
                                    logger.info("   ✅ Свечи обновлены перед обучением")
                                else:
                                    logger.info("   ⏳ Обновление свечей в процессе (может занять время)")
                        except Exception as candles_error:
                            logger.debug(f"   ⚠️ Ошибка обновления свечей: {candles_error}")
                        logger.info("=" * 80)
                        ...
                        if trades_count >= 10:
                            ...
                            logger.info("   💡 Перед обучением на исторических данных обновляем свечи...")
                            try:
                                if self.data_collector:
                                    self.data_collector.load_full_candles_history()
                                    logger.info("   ✅ Свечи обновлены для обучения на исторических данных")
                            except Exception as candles_error:
                                logger.debug(f"   ⚠️ Ошибка обновления свечей: {candles_error}")
