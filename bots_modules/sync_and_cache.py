...
def save_bots_state():
    ...
    try:
        ...
        # При первом раунде на слабых ПК lock занят долго (RSI 552 монет) — даём 180 сек
        import threading
        
        requester = threading.current_thread().name
        lock_timeout = 180.0
        acquired = bots_data_lock.acquire(timeout=lock_timeout)
        if not acquired:
        ...
        try:
            ...
            # ✅ КРИТИЧНО: Не перезаписывать БД пустым списком, если в БД уже есть боты (защита от потери данных)
            if len(bots_data_to_save) == 0:
                try:
                    existing = storage_load_bots_state()
                    if existing and isinstance(existing.get('bots'), dict) and len(existing['bots']) > 0:
                        logger.critical(
                            "[SAVE_STATE] 🚫 ОТМЕНА: попытка сохранить 0 ботов при наличии %s ботов в БД. "
                            "Данные НЕ перезаписаны. Проверьте логи и синхронизацию с биржей.",
                            len(existing['bots'])
                        )
                        return False
                except Exception as check_err:
                    logger.warning("[SAVE_STATE] ⚠️ Не удалось проверить БД перед сохранением пустого состояния: %s", check_err)
    ...
    try:
        ...
        for symbol, bot_snapshot in bots_snapshot.items():
            try:
                ...
                # ✅ Восстановлена логика как до FullAI: всегда обновляем protections (break-even, trailing)
                # Полный Режим ИИ управляет выходом только для своих ботов — не блокирует SL/TP для обычных
                ...
                # ✅ Обновляем стоп-лосс (логика как до FullAI — стопы всегда ставятся)
            ...
            for symbol, bot_data in bot_items:
                    try:
                        ...
                        if symbol in exchange_positions:
                            ...
                            age_sec = (time.time() - entry_sec) if entry_sec else 999
                            if age_sec < _GRACE_SEC:
                                logger.info(f"[SYNC_EXCHANGE] ⏳ {symbol} недавно открыт ({age_sec:.0f}с) — ждём позицию на бирже")
                                continue
