...
def ensure_configs_keys_from_example(project_root: Optional[str] = None) -> bool:
    """
    Создаёт configs/keys.py из примера ТОЛЬКО если файла ещё нет.
    Если configs/keys.py уже есть — НЕ ТРОГАЕМ (никогда не перезаписывать ключи).
    """
    if project_root is None:
        project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    configs_keys = os.path.join(project_root, 'configs', 'keys.py')
    example_keys = os.path.join(project_root, 'configs', 'keys.example.py')
    if os.path.exists(configs_keys):
        return False  # Существующий файл с ключами не трогаем
    if not os.path.isfile(example_keys):
        return False
    os.makedirs(os.path.dirname(configs_keys), exist_ok=True)
    if os.path.exists(configs_keys):  # повторная проверка перед записью
        return False
    try:
