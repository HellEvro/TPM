...
class ContinuousDataLoader:
    ...
    def _continuous_loop(self):
        ...
        time.sleep(5)
        # Под Python 3.13+ free-threaded (no-GIL) потоки реально параллельны — логируем один раз
        try:
            if hasattr(sys, '_is_gil_enabled') and not sys._is_gil_enabled():
                logger.info("🐍 Обнаружен free-threaded Python (no-GIL): этапы 3–4 выполняются параллельно по ядрам")
        except Exception:
            pass
