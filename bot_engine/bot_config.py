...
DEFAULT_AUTO_BOT_CONFIG = {
    ...
    'trailing_stop_activation': 26,   # Процент прибыли, после которого активируется трейлинг
    'trailing_stop_distance': 6.6,      # Дистанция трейлинга от максимальной цены, %
    'trailing_take_distance': 0.56,    # Резервный trailing-тейк (лимит) в %, когда процесс упадет
    ...
    'break_even_trigger': 66,           # Триггер для break even в % (x1 = 100%, но по умолчанию 20%)
    'break_even_trigger_percent': 66,  # Дублирующий ключ для UI (совместимость)
    # Фильтры по тренду
    'trend_detection_enabled': False,    # Включить определение тренда (выключить = пропускает анализ трендов)
