            ...
            with self.lock:
                with self._get_connection() as conn:
                    ...
                    if symbols_to_keep:
                        ...
                        # Если bots_data пустой — НЕ удаляем ботов из БД (защита от случайной потери данных).
                        # Вызывающий код (sync_and_cache.save_bots_state) не должен вызывать сохранение
                        # при 0 ботах, если в БД уже есть боты; здесь — вторая линия защиты.
                        cursor.execute("SELECT COUNT(1) FROM bots")
                        row = cursor.fetchone()
                        count = row[0] if row else 0
                        if count > 0:
                            logger.critical(
                                "🚫 БД: сохранение пустого списка ботов отменено — в таблице bots уже есть %s записей. "
                                "Данные не изменены.", count
                            )
                            return False
                        # БД и так пустая — ничего не делаем
