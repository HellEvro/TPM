...
- 🧵 **Stop-Loss Worker Sync**: фоновый воркер `check_missing_stop_losses()` теперь создаёт `ProtectionState` через `NewTradingBot` + `evaluate_protections` и выставляет биржевые SL/TP по тем же правилам, что и симуляция (см. `bots_modules/sync_and_cache.py`).
