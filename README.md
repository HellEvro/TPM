...
## 🚀 Что нового (февраль 2025)
- 🔌 **Прокси API:** добавлены прокси в app.py для active-detailed, delisted-coins, history, FullAI, analytics — устранены 404 при работе через порт 5000. Таймаут list 30 сек.
- 🔧 **Bybit:** обработка 110001 (order not exists) как успех; лимитные ордера без SL/TP при создании — установка после исполнения.
- 📊 **RSI:** исправлена обработка пустых данных в фильтрах.
- 📄 **Документация:** обновлены ARCHITECTURE, SYSTEM_OVERVIEW, Bots_TZ, DOCUMENTATION_COMPLETE. См. `docs/PRESS_RELEASE_2025-02.md`.
...
- 📦 **Release tooling**: `scripts/sync_to_public.py` формирует `InfoBot_Public` с обязательными стартовыми скриптами и скомпилированными AI файлами, `start_infobot_manager` получил пошаговый лицензионный этап.
...
- 🧵 **Stop-Loss Worker Sync**: `check_missing_stop_losses()` берёт снапшот активных ботов вне `bots_data_lock`, считает `ProtectionState` через `evaluate_protections()` отдельно и лишь кратко обновляет память, поэтому сервер не подвисает даже при массовой установке SL/TP.
...
- 🔁 **Bot Filters Sync**: `bots_modules/filters.py` напрямую использует ядро `bot_engine.filters`, поэтому RSI/ExitScam-фильтры в боевом боте и AI-симуляции дают одинаковые решения без легаси-разночтений.
- 🚦 **Global Toggles Awareness**: AI-фильтры понимают чекбоксы `trading_enabled` и `use_test_server`, благодаря чему симуляции не создают сделки, если торговля выключена или монета переведена в тестовый режим (см. `bot_engine/ai/filter_utils.py`).
- 🧬 **Optimizer Genomes**: `ai_strategy_optimizer` читает диапазоны параметров и лимиты прогонов из `data/ai/optimizer_genomes.json`, логирует полный набор параметров и диффы при сохранении индивидуальных настроек — контроль оптимизации стал прозрачным.
- 📊 **Optimizer Insights API**: `/api/ai/optimizer/results` и панель в BotsManager отображают свежие глобальные параметры, топ монет по рейтингу и статистику паттернов.
- 🧪 **Protection Parity Tests**: `tests/test_ai_simulator_parity.py` теперь прогоняет полные серии LONG/SHORT цен и доказывает, что AI-симуляция и реальный бот закрывают позиции по одинаковым причинам и таймингам.
- 🧪 **AI Trainer Smoke Tests**: `tests/test_ai_individual_settings.py` проверяет, что тренер сохраняет полный набор ключей (RSI, фильтры, риск, AI телеметрия) перед публикацией индивидуальных настроек.
...
- **Релизы:** `scripts/sync_to_public.py`, каталог `InfoBot_Public/`, авто-добавление `start_infobot_manager.{cmd|sh|vbs}`, `launcher/`.
...
1. Установите **Python 3.12+** (рекомендуется 3.14+) (Windows/Mac/Linux).
...
3. Далее GUI сделает всё автоматически:
   - создаст/обновит виртуальное окружение и поставит зависимости (включая PyTorch);
   - установит PyTorch с GPU поддержкой (если GPU доступен);
   ...
   - запустит `app.py`, `bots.py`, `ai.py`, покажет статусы сервисов и логи.
...
# Установка PyTorch с GPU (если GPU доступен)
