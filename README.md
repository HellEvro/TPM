...
- 🛡️ **Protection Engine 1.0**: вынесенная логика стоп-лосс/безубыток/трейлинг/лимит времени (`bot_engine/protections.py`) подключена к AI симуляциям и постепенно замещает внутренние проверки `NewTradingBot`, что устраняет расхождения между реалом и тренером.
...
## 📚 Документация
- 📦 `docs/INSTALL.md` — требования, установка, решение типовых проблем.
