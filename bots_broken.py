...
# Отключаем werkzeug логгер (Flask HTTP логи)
werkzeug_logger = logging.getLogger('werkzeug')
werkzeug_logger.setLevel(logging.ERROR)  # Показываем только ошибки
werkzeug_logger.disabled = False  # Но не отключаем полностью

# Создаем фильтр для подавления HTTP запросов
class NoHTTPFilter(logging.Filter):
    def filter(self, record):
        message = record.getMessage()
        # Блокируем все HTTP запросы
        if 'HTTP/1.' in message and '" ' in message:
            return False
        return True

# Применяем фильтр
http_filter = NoHTTPFilter()
werkzeug_logger.addFilter(http_filter)
...
def init_bot_service():
    ...
    try:
        ...
        # 4. Запускаем воркеры (с обработкой ошибок)
        ...
        try:
            ...
            logger.error(f"[INIT] ⚠️ Ошибка запуска воркеров: {e}")
            logger.info("[INIT] Продолжаем без воркеров...")
        ...
        # Устанавливаем флаг инициализации ПОСЛЕ всех операций
        ...
        def run_flask_server():
            try:
                ...
                app.run(
                    host='0.0.0.0',
                    port=5001,
                    debug=False,  # Отключаем debug чтобы не было лишних логов
                    use_reloader=False,
                    threaded=True
                )
            except Exception as e:
