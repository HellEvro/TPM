...
При каждом запуске **лаунчера** (InfoBot Manager) и при запуске **app.py** выполняется механизм патчей: проверяется список ещё не применённых патчей из папки `patches/patches/`, каждый такой патч запускается один раз. Результат сохраняется в `launcher/.patches_applied.json`, чтобы не применять один и тот же патч повторно.

## Правило: все изменения в коде — через патчи

**Любые правки в репозитории (фичи, фиксы, миграции), затрагивающие общие файлы проекта, должны быть оформлены патчами.** Не править напрямую `app.py`, `static/`, `templates/` и т.д. для «обновлений системы» — создавать новый файл в `patches/patches/` с функцией `apply(project_root) -> bool`. Так все пользователи (и свежие клоны) после запуска получают одинаковое состояние без ручного копирования правок.
