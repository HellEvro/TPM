...
class LogLevelFilter(logging.Filter):
    ...
    def filter(self, record):
        ...
        # Скрываем сообщения с неформатированными плейсхолдерами %s, %d и т.д. (обычно это ошибки форматирования)
        # ВАЖНО: Эта проверка должна быть ПЕРВОЙ, чтобы скрывать такие сообщения независимо от уровня
        try:
            message = record.getMessage() if hasattr(record, 'getMessage') else str(record.msg)
            if isinstance(message, str):
                import re
                # Если getMessage() возвращает сообщение с неформатированными плейсхолдерами,
                # это означает, что сообщение не было правильно отформатировано
                # Скрываем все сообщения, содержащие неформатированные плейсхолдеры: %s, %d, %f, %r, %x, %o, %X, %c, %i, %u, %e, %E, %g, %G
                # НО НЕ скрываем правильно отформатированные сообщения типа %(name)s
                
                # Проверяем наличие неформатированных плейсхолдеров (не в формате %(name)s)
                # Паттерн для поиска неформатированных плейсхолдеров: %[опциональные флаги и ширина][опциональная точность]тип
                # Где тип: s, d, i, o, u, x, X, e, E, f, F, g, G, c, r
                unformatted_pattern = r'%[0-9]*\.?[0-9]*[sdioxXucrfeEgG]'
                
                # Проверяем, есть ли неформатированные плейсхолдеры
                matches = re.findall(unformatted_pattern, message)
                if matches:
                    # Проверяем, что это не правильно отформатированные сообщения типа %(name)s
                    # Правильно отформатированные сообщения имеют формат %(name)s, а не просто %s
                    has_named_format = re.search(r'%\([^)]+\)[sdioxXucrfeEgG]', message)
