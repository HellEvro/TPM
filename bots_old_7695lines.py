...
# State Manager (новая архитектура) - опционально доступен
bot_system_state = None  # BotSystemState instance (если инициализирован)
...
# Счетчик для уменьшения спама логов при ошибках синхронизации
_sync_error_counter = 0

def sync_positions_with_exchange():
    """Умная синхронизация позиций ботов с реальными позициями на бирже"""
    global _sync_error_counter
    ...
    try:
        ...
        if not exchange_positions:
            _sync_error_counter += 1
            # Логируем только каждый 30-й раз чтобы не спамить
            if _sync_error_counter % 30 == 0:
                logger.warning(f"[POSITION_SYNC] ⚠️ Не удалось получить позиции с биржи ({_sync_error_counter} раз)")
            return False
        
        # Сбрасываем счетчик при успешном получении позиций
        if _sync_error_counter > 0:
            logger.info(f"[POSITION_SYNC] ✅ Связь с биржей восстановлена (после {_sync_error_counter} ошибок)")
            _sync_error_counter = 0
